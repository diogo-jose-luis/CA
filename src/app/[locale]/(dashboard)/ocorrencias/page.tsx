...
const mockData = [
  {
    ...
    tipo: "Cliente",
    categoria: "Acidente",
    descricao: "Queda de colaborador na área de carga",
    risco: "Alto",
    periodo: "Diurno",
  },
  {
    ...
    tipo: "Segurança",
    categoria: "Furto",
    descricao: "Tentativa de furto no parque",
    risco: "Médio",
    periodo: "Noturno",
  },
  {
    ...
    tipo: "Cliente",
    categoria: "Roubo",
    descricao: "Roubo de material no armazém",
    risco: "Alto",
    periodo: "Diurno",
  },
];
...
function RiskBadge({ risk }: { risk: string }) {
  ...
  return (
    <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${map[risk]}`}>
      {risk}
    </span>
  );
}
      ...
      <div className="ca-card p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
          ...
          <select className="ca-input">
            ...
            <option>{t("types.client")}</option>
            <option>{t("types.security")}</option>
          </select>

          <select className="ca-input">
            <option>{t("filters.category")}</option>
            <option>{t("categories.accident")}</option>
            <option>{t("categories.theft")}</option>
            <option>{t("categories.robbery")}</option>
          </select>

          <select className="ca-input">
            <option>{t("filters.period")}</option>
            <option>{t("period.day")}</option>
            <option>{t("period.night")}</option>
          </select>
        ...
        <table className="w-full text-sm">
          ...
          <thead className="bg-slate-50 dark:bg-slate-800/40">
            ...
            <tr>
              ...
              <th className="py-3">{t("table.type")}</th>
              <th className="py-3">{t("table.category")}</th>
              <th className="py-3">{t("table.description")}</th>
              <th className="py-3">{t("table.risk")}</th>
              <th className="py-3">{t("table.period")}</th>
          ...
          <div className="relative ml-auto h-full w-full max-w-md ca-panel shadow-2xl flex flex-col">
            ...
            <form className="p-4 space-y-4 flex-1 overflow-y-auto ca-scroll">
              ...
              <textarea
                className="ca-input"
                rows={3}
                placeholder={t("form.actionTaken")}
              />

              <textarea
                className="ca-input"
                rows={3}
                placeholder={t("form.recommendations")}
              />
